...
const Feedback = (): JSX.Element => {
  ...
  const { rerponse, addDocument, deleteDocument, editDocument } = useFirebase('feedback');
  // 입력창 관련 state
  ...
  // 피드백 목록 관련 state
  const [feedbackList, setFeedBackList] = useState<FeedBackWidthEdit[]>([]);
  ...
  // 편집 변경 목록 업데이트
  useEffect(() => {
    const list:FeedBackWidthEdit[] = [];
    documents?.forEach(item => {
      // 추가적인 속성을 추가(edit)
      const tempFeedback:FeedBackWidthEdit = {...item, edit:false};
  ...
  const editChangeMode = (_id: string, _isEdit:boolean) => {
    const arr: FeedBackWidthEdit[] = feedbackList.map(item => {
      if (item.id === _id) {
        // 기존 message
